...
import { BellRing, CheckCircle, XCircle, User as UserIcon, Loader2, RefreshCw } from 'lucide-react';
  ...
  const fetchCurrentTicket = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // Check if ticket belongs to this teller (in a real app, backend would filter this)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col h-[450px]">
          <div className="flex justify-between items-center border-b pb-4 mb-4">
             <h3 className="text-lg font-semibold text-gray-800">Phiên phục vụ hiện tại (Staff: {user?.fullName || 'N/A'})</h3>
             {currentTicket && (
          ...
          <div className="grid grid-cols-3 gap-4 mt-8">
            <button 
              ...
              onClick={() => updateStatus(5)} // Cancelled/Skip
            ...
            <button 
              disabled={!currentTicket || actionLoading}
              className="flex items-center justify-center gap-2 py-4 bg-vietin-red/10 hover:bg-vietin-red/20 text-vietin-red rounded-xl font-bold transition-all active:scale-95 disabled:opacity-50"
            >
              <BellRing className="w-6 h-6" /> Gọi lại
            </button>
            <button 
              disabled={!currentTicket || actionLoading}
              onClick={() => updateStatus(4)} // Completed
